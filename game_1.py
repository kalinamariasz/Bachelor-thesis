...
class Game_1:
    def __init__(self, root):
        ...
        self.computer_numbers = [2, 20, 49]
        self.player_numbers = [7, 14, 24]
        self.player_hand = []
        self.computer_hand = []
        self.current_card = 0
    ...
    def computer_wait(self):
        ...
        # EXCEPTION: Computer waits for the player to play 14 to play 20
        if card == 20 and self.current_card == 14:
                self.wait = self.current_frame.after(6000, lambda: self.play_card_computer())
        # EXCEPTION: Computer waits only 0.7s to play the first card
        if card == 2 and self.current_card == 0:
            self.wait = self.current_frame.after(700, lambda: self.play_card_computer())
        # If it's higher than the current card, start waiting to play your card
        elif card > self.current_card and card != 20 and card != 2:
    ...
    def ask_question(self):
        ...
        with open('Experiment_data_' + str(self.participant_info.id) + '.csv', 'a', newline='') as f:
            ...
            writer.writerow({"ID": '-', "Age": '-', "Game number": 1, "Previous card": self.previous_card, "Played card": self.current_card, "Time elapsed": elapsed_time, "Time estimated": answer, "Cards participant": self.player_numbers, "Number of cards participant": len(self.player_numbers), "Cards computer": self.computer_numbers, "Number of cards computer": len(self.computer_numbers)})
    ...
    def lower_card_player(self):
        ...
        if len(self.player_numbers)>0:
            if self.current_card > min(self.player_numbers):
                ...
                with open('Experiment_data_' + str(self.participant_info.id) + '.csv', 'a', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=self.participant_info.fieldnames)
                    writer.writerow({"ID": '-', "Age": '-', "Game number": 1, "Previous card": self.previous_card, "Played card": self.current_card, "Time elapsed": 'mistake', "Time estimated": 'mistake', "Cards participant": self.player_numbers, "Number of cards participant": len(self.player_numbers), "Cards computer": self.computer_numbers, "Number of cards computer": len(self.computer_numbers)})
