...
class Game_2:
    def __init__(self, root):
        ...
        self.computer_numbers = [25, 35, 85]
        self.player_numbers = [10, 55, 70]
        self.player_hand = []
        self.computer_hand = []
        self.current_card = 0
    ...
    def ask_question(self):
        ...
        with open('Experiment_data_' + str(self.participant_info.id) + '.csv', 'a', newline='') as f:
            ...
            writer.writerow({"ID": '-', "Age": '-', "Game number": 2, "Previous card": self.previous_card, "Played card": self.current_card, "Time elapsed": elapsed_time, "Time estimated": answer, "Cards participant": self.player_numbers, "Number of cards participant": len(self.player_numbers), "Cards computer": self.computer_numbers, "Number of cards computer": len(self.computer_numbers)})
    ...
    def lower_card_player(self):
        ...
        if len(self.player_numbers)>0:
            if self.current_card > min(self.player_numbers):
                ...
                with open('Experiment_data_' + str(self.participant_info.id) + '.csv', 'a', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=self.participant_info.fieldnames)
                    writer.writerow({"ID": '-', "Age": '-', "Game number": 2, "Previous card": self.previous_card, "Played card": self.current_card, "Time elapsed": 'mistake', "Time estimated": 'mistake', "Cards participant": self.player_numbers, "Number of cards participant": len(self.player_numbers), "Cards computer": self.computer_numbers, "Number of cards computer": len(self.computer_numbers)})
